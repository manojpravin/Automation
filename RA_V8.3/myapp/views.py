...
def UploadSuccess(request):
    ...
    my_connect = create_engine("mysql+mysqldb://root:admin123@localhost/logindata?charset=utf8mb4") #fill details
    print("Connection Established")
    
    before_csv = pd.read_csv(request.FILES["Before"],encoding='utf-8')
    ...
    Active = my_connect.execute("SELECT * FROM logindata.activeusers where ReportWeek={0} and ReportYear = {1}".format(weekNumber,year)).fetchall()
    loggedin = my_connect.execute("SELECT * FROM logindata.loggedinusers where ReportWeek={0} and ReportYear = {1}".format(weekNumber,year)).fetchall()
